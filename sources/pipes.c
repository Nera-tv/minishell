...
/*   Updated: 2022/12/03 17:20:19 by tweidema         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

void	redirect_pipeout(t_data	*data)
{
...
void	redirect_pipein(t_data	*data)
{
...
{
	if (val == 0)
		redirect_pipeout(data); //Rajouter pour les < ou << vu que c'est la première commande
	else if (val == data->nbr_cmds - 1)
		redirect_pipein(data); // rajouter les > ou >> vu que c'est la dernière commande
	else // rajouter ici le check des 2 au dessus
	{
		redirect_pipein(data);
		redirect_pipeout(data);
	}
	return (0);
}
